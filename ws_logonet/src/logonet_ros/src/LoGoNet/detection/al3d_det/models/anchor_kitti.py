...
class ANCHORKITTI(nn.Module):
    ...
    def get_training_loss(self):
        disp_dict = {}
        tb_dict = {}
        loss, tb_dict = self.dense_head.get_loss() # detection/al3d_det/models/modules/dense_heads/anchor_head_template.py
    ...
    def post_processing(self, batch_dict):
        ...
        for index in range(batch_size):
            if batch_dict.get('batch_index', None) is not None:
                ...
                batch_mask = index # batch_mask就是指示batch中哪一个的索引
 
            box_preds = batch_dict['batch_box_preds'][batch_mask]
            src_box_preds = box_preds

            
            if not isinstance(batch_dict['batch_cls_preds'], list): # 执行这里
                ...
                if not batch_dict['cls_preds_normalized']:
                    cls_preds = torch.sigmoid(cls_preds) # 使用sigmoid 进行标准化
            else:
            ...
            # 执行这里
                ...
                if selected_scores.numel() > 0:
                    ...
                    max_score_indices = (selected_scores == max_score).nonzero(as_tuple=True)[0]

                    if max_score_indices.numel() > 1:
                        # 如果有多个分数相同的框，选择面积最大的框
                        box_areas = (box_preds[selected[max_score_indices], 3] - box_preds[selected[max_score_indices], 1]) * \
                                    (box_preds[selected[max_score_indices], 4] - box_preds[selected[max_score_indices], 2])
                        max_area_idx = box_areas.argmax()
                        max_score_idx = max_score_indices[max_area_idx]
                        
                        # 确保最终只保留一个框 (如果还多于一个框)
                        if len(max_score_idx) > 1:
                            max_score_idx = max_score_idx[0]  # 选择第一个得分最高的框
                    else:
                        # 只有一个最高分数的框
                        max_score_idx = max_score_indices.item()

                    # 确定最终的 `final_scores`、`final_labels` 和 `final_boxes`
                    final_scores = selected_scores[max_score_idx].unsqueeze(0)
                    final_labels = label_preds[selected[max_score_idx]].unsqueeze(0)
                    final_boxes = box_preds[selected[max_score_idx]].unsqueeze(0)
                else:
                    # 没有满足条件的框时，保持空
    ...
    def generate_recall_record(box_preds, recall_dict, batch_index, data_dict=None, thresh_list=None):
        ...
        rois = data_dict['rois'][batch_index] if 'rois' in data_dict else None # 一阶段的ROI框
        
        gt_boxes = data_dict['gt_boxes'][batch_index] # 提取真实框
        # 初始化recall_dict
